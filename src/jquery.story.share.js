...
;(function($, window, document, undefined) {
  ...
  var $el,
      ...
      defaults = {
        ...
        mediaBaseUrl: window.location.href.substring(0,
  ...
  Plugin.prototype = {
    ...
    handleWindowOverrides: function() {

      if ('windowOverrides' in this.options.socialProviders[this.type]) {
        this.options = $.extend(true, {}, this.options, this.options.socialProviders[this.type].windowOverrides);
      }
    ...
    linkHandler: function() {
      ...
      var px = Math.floor(
              ((screen.availWidth || 1024) - this.options.windowWidth) / 2),
          py = Math.floor(
              ((screen.availHeight || 700) - this.options.windowHeight) / 2);

      this.setType();
      this.handleWindowOverrides();
      ...
      var popup = window.open(this.socialUrl, 'social',
          'width=' + this.options.windowWidth + ',height=' +
    ...
    validateAttributes: function() {
      ...
      if (defaults.requiredLocalAttributes[this.type] &&
          Array.isArray(defaults.requiredLocalAttributes[this.type])) {

        for (var i = 0; i <
        defaults.requiredLocalAttributes[this.type].length; i++) {
          if (!$el.attr(
                  'data-' + defaults.requiredLocalAttributes[this.type][i])) {
            this.socialUrl = null;
            throw '[Story-Share] Missing ' +
            defaults.requiredLocalAttributes[this.type][i] + ' for ' +
            this.type;
          }
      ...
      if (defaults.requiredGlobalAttributes[this.type] &&
          Array.isArray(defaults.requiredGlobalAttributes[this.type])) {

        for (var x = 0; x <
        defaults.requiredGlobalAttributes[this.type].length; x++) {
          if (!this.options[defaults.requiredGlobalAttributes[this.type][x]]) {
            this.socialUrl = null;
            throw '[Story-Share] Missing ' +
            defaults.requiredGlobalAttributes[this.type][x] + ' for ' +
            this.type;
          }
        }
    ...
    setType: function() {
      ...
      if ($el.attr('data-type')) {
        this.type = $el.attr('data-type').replace('-', '_');
      } else {
    ...
    generateSocialUrl: function() {
      ...
      var parentObject = this,
          socialProvider = this.type in this.options.socialProviders ?
              this.options.socialProviders[this.type] :
              null;

      if (null === socialProvider) {
        throw '[Story-Share] Type ' + this.type + ' is not supported';
      }
      ...
      tokens.forEach(function(element) {
        ...
        if (parentObject.options.relativeMediaUrls && dataAttribute &&
            this.type in parentObject.options.mediaAttributes) {

          if (parentObject.options.mediaAttributes[this.type].indexOf(
                  processedElement) > -1) {
        ...
        if (dataAttribute === parentObject.options.currentUrlPlaceholder &&
            parentObject.options.shareUrlAttributes[this.type].indexOf(
                processedElement) > -1) {
        ...
        if (null === dataAttribute) {
          socialUrl = parentObject.removeURLParameter(socialUrl,
              attributeName.replace('data-', ''));
        }

        socialUrl = socialUrl.replace('{' + element + '}',
            encodeURIComponent(dataAttribute));
      }.bind(this));
